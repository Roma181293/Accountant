...
class CurrencyTests: XCTestCase {
    ...
    func testAddCurrency() throws {
        let currency = try Currency.createAndGet(code: "AUD", iso4217: 036,
    ...
    func testCreateDublicatedCurrency() throws {
        try Currency.create(code: "XXX",
                            iso4217: 036,
                            name: "XXXX",
                            createdByUser: false,
                            context: context)
        XCTAssertThrowsError(try Currency.create(code: "XXX",
                                                 iso4217: 086,
                                                 name: "XXXX",
                                                 createdByUser: false,
                                                 context: context),
    ...
    func testSetAccountingCurrency() throws {
        try Currency.create(code: "AUD",
                            iso4217: 036,
                            name: "Австралійський долар",
                            createdByUser: false,
                            context: context)
        guard let currency = try Currency.getCurrencyForCode("AUD", context: context)
        else {
            print("Currency should not be nil")
            XCTAssertTrue(false)
            return
    ...
    func testFetchCount() throws {
        ...
        let currencies = try coreDataStack.persistentContainer.viewContext.fetch(fetchRequest)
