    ...
    func configureUI() { // swiftlint:disable:this function_body_length
        ...
        if let credit = controller.credit, let debit = controller.debit {
            if debit.currency == nil
                || (controller.debit?.type.allowsTransactions == false
                    && debit.type.name != AccountType.NameEnum.capitalConsolidation.rawValue)
                || credit.currency == nil
                || (credit.type.allowsTransactions == false
                    && credit.type.name != AccountType.NameEnum.capitalConsolidation.rawValue) {
