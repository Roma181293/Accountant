...
class AccountEditorAssembly {
    class func configureCreateMode(parentAccountId: UUID) -> AccountEditorViewController {
        ...
        let worker = AccountEditorWorker(persistentContainer: CoreDataStack.shared.persistentContainer,
                                           parentAccountId: parentAccountId)
        
        let router = AccountEditorRouter()
        let viewController = AccountEditorViewController()
        let interactor = AccountEditorInteractor(service: worker)
    ...
    class func configureEditMode(accountId: UUID) -> AccountEditorViewController {
        ...
        let service = AccountEditorWorker(persistentContainer: CoreDataStack.shared.persistentContainer,
                                           accountId: accountId)
