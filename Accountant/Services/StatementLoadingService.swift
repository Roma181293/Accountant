...
class StatementLoadingService {
    static func loadStatments(context: NSManagedObjectContext, compliting: @escaping(Bool?, Error?) -> Void) {
        ...
        for item in bankAccounts {
            ...
            guard let bankAccount = BankAccount.getBankAccountByExternalId(item.externalId!, context: newContext)
            else {return}
                ...
                NetworkServices.loadStatementsForBankAccount(bankAccount, startDate: startDate, endDate: endDate,
                    ...
                    if let statments = statments {
                        do {
                            ...
                            statments.forEach({
                                transactions.append(Transaction.addTransactionDraft(account: bankAccount.account!,
